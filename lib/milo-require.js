...
function MiloRequire(jsPrefix, version) {
	...
	this.load = function (src, dependencies, callback) {
		...
		if (script.element === null) {
			script.element = document.createElement(script.src.endsWith('html') ? 'noscript' : 'script');
	...
	this.resolveSrc = function (src) {
		if (this.jsPrefix && src.substr(0,1) !== "/" && src.substr(0,4) !== "http") {
			src = window.location.pathname + this.jsPrefix + src;
		}
